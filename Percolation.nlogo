...
patches-own [ 
  porosity        ;; probability of percolation per patch
  ]

;; color code
    ;; grey    porosity of  patch
    ;; black   non-porous patch
    ;; blue    percolated patch
    ;; red     patches that can percolate
...
to setup
  ...
  ask patches [
    reset-porosity
    set pcolor scale-color grey porosity 0 100
  ]
  ;; set up top row
...
;; This procedure sets the porosity for the patch, according to the checkerboard pattern.
to reset-porosity ;; patch procedure
    ifelse (pxcor + pycor) mod 2 = 1
      [ set porosity random-normal average-porosity 10 ]
      [ set porosity 0 ]
end
...
to go
  if not any? current-row with [pcolor = red]
    [ stop ]
  if (count patches with [pcolor = blue]) >= max-oil
    [ stop ]
  percolate
  tick
end
...
to percolate
  ask current-row with [pcolor = red] [
    ...
    ask patches at-points [[-1 -1] [1 -1]]
      [ if random-float 100 < porosity ;; with probability p percolate
          [ set pcolor red ] ]
    set pcolor blue
  ]
